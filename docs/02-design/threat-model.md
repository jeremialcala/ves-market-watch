...
| T12 | ADR-0012; ADR-0017 (`apps/web-spa`) | ✔ Cubierto: `cacheLocation: memory` + rotation en `AuthProvider` (revisado; guard con tests); checklist e2e verifica en DevTools que no hay tokens en storage; CSP del nginx. **Hueco conocido (2026-07-31)**: la re-autenticación silenciosa por iframe (`prompt=none`) necesita `frame-src` del tenant y la CSP solo declara `default-src 'self'` — nunca se ejercitó porque el e2e con login real sigue pendiente del `client_id` (F1 de ADR-0017) |
