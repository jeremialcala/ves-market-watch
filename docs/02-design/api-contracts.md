...
`GET /analysis/current` y el tópico `analysis`).
...
| `/analysis/current` | GET | `currency` (def. `VES`) | lectura de los medidores: banda, escala con sus cortes, posición y proximidad a cada regla | read:indicators |
...
| `analysis.updated` | indicator-engine | api-gateway (`GET /analysis/current` + push WSS) | `schemas/analysis.v1.json` (RF-6/ADR-0019, ambos lados 2026-08-01) |
