...
(fase 03). El contrato `signal.v1` ya está definido (2026-07-20), pero el evento
`signals.emitted` **aún no se emite**: falta el motor de reglas (RF-4) — ver la fase de
señales pendiente en `motor-indicadores.md`.
...
| `signals.emitted` | indicator-engine | api-gateway | `schemas/signal.v1.json` (contrato definido; emisión pendiente del motor de reglas RF-4) |
